...
class OrcamentoController extends BaseRestController
{
    ...
    public function actionIndex()
    {
        ...
        if($user->role_id == 3){
            ...
            throw new ForbiddenHttpException("Não tem permissão para ver os orçamentos.");
        }
    }
    ...
    {                
        ...
        if($user->role_id == 1 || $user->role_id == 3){
            throw new ForbiddenHttpException("Não tem permissão para criar orçamentos.");
        }
        // Pegando os dados do corpo da requisição
        $post = $this->request->post();
        // Verificar se os campos necessários estão presentes no corpo da requisição
        if (empty($post['descricao']) || empty($post['servicoOrcamento'])) {
            throw new BadRequestHttpException("Faltam campos obrigatórios. descrição ou servicoOrcamento.");
        }
        ...
        if ($model->save()) {
            ...
            for ($i = 0; $i < count($post['servicoOrcamento']); $i++) {
                $servicoOrcamento = new ServicoOrcamento();
                $servicoOrcamento->orcamento_id = $model->id;
                $servicoOrcamento->servico_id = $post['servicoOrcamento'][$i]["servico_id"];
                $servicoOrcamento->quantidade = $post['servicoOrcamento'][$i]["quantidade"];
    ...
    public function actionUpdate($idOrcamento)
    {
        // Obter o token da autorização dos cabeçalhos da solicitação
        $authorizationHeader = Yii::$app->getRequest()->getHeaders()->get('Authorization');
        $user = User::findByAccessToken($authorizationHeader);
        $utilizador = Utilizador::find()->where(['user_id' => $user->id])->one(); 
        $model = Orcamento::findOne($idOrcamento);
        
        if ($model === null) {
            throw new NotFoundHttpException("O orçamento com ID $idOrcamento não foi encontrado.");
        }
        if($user->role_id == 1 || $utilizador -> idLab == $model->laboratorio_id || $utilizador->id == $model->utilizador_id ) {
            ...
            if ($model->save()) {
                $estadoOrcamento = new EstadoOrcamento();
                $estadoOrcamento->orcamento_id = $model->id;
                $estadoOrcamento->estado_id = $requestData['estadoOrcamento'];
                $estadoOrcamento->data = date('Y-m-d'); // Formato apenas com dia, mês e ano
                $estadoOrcamento->save();
                ...
                for ($i = 0; $i < count($requestData['servicoOrcamento']); $i++) {
                    $servicoOrcamento = new ServicoOrcamento();
                    $servicoOrcamento->orcamento_id = $model->id;
                    $servicoOrcamento->servico_id = $requestData['servicoOrcamento'][$i]["servico_id"];
                    $servicoOrcamento->quantidade = $requestData['servicoOrcamento'][$i]["quantidade"];
                    $servicoOrcamento->save();
                }
                return $model;
            } else {
